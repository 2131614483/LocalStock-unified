...
def apply_qclaw_provider_to_settings(
    ...
    *,
    ...
    # NOTE: context_window is a user preference (context-usage display only),
    # not a gateway parameter — deliberately NOT taken from ``resolved`` so
    # auto-configuration never resets what the user chose in AI 模型设置.

...
# ── Strategy routing cheat-sheet (offline packs only) ─────────────────────────
# The offline pack ships the FULL strategy library because no Stage 1 diagnosis
# exists at export time — the model produces it itself in part 1.  This table
# mirrors pa_agent/ai/router.py so the model can self-route from its own
# Stage 1 output.  Keep the two in sync when routing rules change.

_STRATEGY_ROUTING_TABLE = """## 策略文件路由表（离线包专用：请你自己路由）

本离线包**全量**装载了所有策略文件。请**先**完成阶段一诊断，再按下面的规则
**只采用匹配的策略文件**；其余文件对应的市场状态不成立，其入场规则一律忽略。

### 第一步：按 cycle_position + direction 选「基础文件」

| cycle_position | direction | 采用的文件 |
|---|---|---|
| spike | bullish | 极速上涨分析识别 + 极速上涨交易策略 |
| spike | bearish | 极速下跌分析识别 + 极速下跌交易策略 |
| spike（spike_stage=ending） | 任意 | 上述极速文件 + 对应方向的通道文件 |
| spike（spike_stage=transitioning） | 任意 | 只按通道态处理（见下面通道行） |
| micro_channel / tight_channel / normal_channel / broad_channel | bullish | 上涨通道分析识别 + 上涨通道交易策略 + 文件13 |
| micro_channel / tight_channel / normal_channel / broad_channel | bearish | 下跌通道分析识别 + 下跌通道交易策略 + 文件13 |
| micro_channel / tight_channel / normal_channel / broad_channel | neutral | 震荡区间分析识别 + 震荡区间交易策略 |
| trending_tr / trading_range | 任意 | 震荡区间分析识别 + 震荡区间交易策略 |
| **extreme_tr / unknown** | 任意 | **不采用任何策略文件 → 只能输出「不下单」** |

补充规则：

- `micro_channel` 且 `spike_stage` 为 active / ending 时，**额外采用**对应方向的极速文件。
- `alternative_cycle_position` 非空且与主状态不同时，**同时采用**备选状态的基础文件。
- `trend_context.recent_spike` 与 direction 一致时，额外叠加对应方向的极速文件。

### 第二步：按 detected_patterns 叠加文件

| 触发条件 | 追加采用 |
|---|---|
| wedge | 文件14-楔形形态分析交易 |
| mtr | 文件25-主要趋势反转MTR |
| final_flag | 文件24-最终旗形与趋势末端 |
| 通道态，或 reversal_attempt / mtr / final_flag / h2 / l2 | 文件15-二次入场机会 |
| 通道态，或 h1 / h2 / l1 / l2 | 文件19-H1H2-L1L2计数 |
| breakout_failure / failed_breakout / breakout_test / breakout_pullback | 文件18-突破失败与突破测试 |
| always_in / ail / ais / 20gb / gap_bar | 文件20-AlwaysIn与20GB |
| 区间态，或 barbwire / wire / overlap / middle_range | 文件21-铁丝网与无交易环境 |
| failed_signal / breakout_failure / failed_breakout / magnet / trapped_traders | 文件22-信号失败后的磁力位 |
| ascending_triangle / descending_triangle / symmetrical_triangle / expanding_triangle | 文件27-三角形与收敛形态 |
| double_top_bottom | 文件28-双重顶底与微型结构 |

### 第三步：永远采用的基础文件

逐棒分析检查单、文件16-K线信号识别、文件17-止损和止盈与仓位管理、文件23-MeasuredMove与结构目标

### ⚠️ 方向冲突过滤

`direction=bullish` 时**忽略**全部「下跌通道 / 极速下跌」文件；
`direction=bearish` 时**忽略**全部「上涨通道 / 极速上涨」文件。
"""



def render_strategy_routing_table() -> str:
    """Return the offline self-routing cheat-sheet (see router.py for the code path)."""
    return _STRATEGY_ROUTING_TABLE
...
class PromptAssembler:
    ...
    def _include_symbol_name(self) -> bool:
        """Whether to append the instrument's display name next to its code."""
        cfg = self._prompt_settings
        if cfg is None:
            return False
        return bool(getattr(cfg, "include_symbol_name", False))

    def instrument_label(self, frame: KlineFrame) -> str:
        """``600519（贵州茅台）`` when enabled and a name was resolved, else the code.

        The name is resolved by the data source and only carried on the frame when
        the user enabled 「让 AI 知道品种名称」; with no name we degrade to the bare
        symbol so prompts are unaffected.
        """
        symbol = str(getattr(frame, "symbol", "") or "")
        name = str(getattr(frame, "symbol_name", "") or "").strip()
        if not name or not self._include_symbol_name():
            return symbol
    ...
    def _build_stage1_user_prompt(self, frame: KlineFrame, *, analysis_mode: str = "original") -> str:
        ...
        return (
            ...
            "---\n\n"
            f"## 当前分析目标\n\n"
            f"品种:{self.instrument_label(frame)} 周期:{frame.timeframe} K线数量:{n_bars}\n"
    ...
    def _build_stage2_user_prompt(
        ...
        *,
        ...
        if omit_kline_block:
            ...
            simple_features_block = self._render_simple_market_features_block(frame)
            kline_block = (
                f"## K线数据 · {self.instrument_label(frame)} · 共{n_bars}根"
                f"（含阳阴列；各节点 bar_range 由你据实填写）\n\n"
                f"{kline_table}\n\n"
    ...
    # ── Offline (no-API) pack builders ────────────────────────────────────────
    # These mirror the API path but ship the FULL strategy library, because the
    # routing decision (which depends on the Stage 1 diagnosis) is not available
    # at export time.  The model self-routes from its own Stage 1 output using
    # the cheat-sheet prepended to the Stage 2 turn.

    def stage1_system_prompt_only(self) -> str:
        """Return the shared system prompt used by Stage 1 requests."""
        return self._build_stage1_system_prompt()

    def build_offline_stage1_user(self, frame: KlineFrame) -> str:
        """Stage-1 task turn for the offline pack (identical to the API path)."""
        return self._build_stage1_user_prompt(frame, analysis_mode="original")

    def build_offline_stage2_user(
        self,
        frame: KlineFrame,
        *,
        ...
        """Stage-2 task turn without a Stage-1 diagnosis (model self-routes)."""
        stance_block = build_decision_stance_guidance(normalize_stance(decision_stance))
        stage2_parts = [
            stance_block,
            render_strategy_routing_table(),
            *(
                self._load(name)
                for name in stage2_user_task_txt_files(load_full_strategy_library=True)
            ),
            _STAGE2_OUTPUT_CONTRACT,
            (
                _NEXT_BAR_PREDICTION_INSTRUCTION
                if enable_next_bar_prediction
                else _NEXT_BAR_DISABLED_NOTE
            ),
            _build_next_cycle_prediction_instruction(
        ...
        stage1_slot = (
            "## 阶段一诊断结果\n\n"
            "本离线包**不含**阶段一结果——阶段一由你（大模型）在第 1 部分中产出。\n\n"
            "- 若你已在**同一对话**中回复过阶段一 JSON：直接以你自己上一条回复的 JSON "
            "为权威依据，无需用户再次提供。\n"
            "- 若这是**新开的对话**：请提示用户把阶段一诊断 JSON 粘贴到下方代码块后再继续。\n\n"
            "```json\n{ 在此粘贴阶段一诊断 JSON；同一对话时可留空，以上一条回复为准 }\n```\n\n"
        )
        ...
        return (
            ...
            "## 阶段二任务\n\n"
            "你现在独立执行阶段二：交易决策、风险收益和下单方式评估（基于阶段一诊断结果）。\n"
            "请先读上方的「策略文件路由表」，按阶段一诊断结果**自行选定**适用的策略文件；"
            "本消息下方附有完整 K 线表与几何特征。\n\n"
            f"{stage2_context}\n\n"
            "---\n\n"
            f"{stage1_slot}"
            ...
            f"(含 decision_trace 与 terminal)。\n"
            f"注意:如果判断不下单,entry_price、take_profit_price、stop_loss_price、"
            f"order_direction 必须全部为 null。\n\n"
            f"{_STAGE2_TAIL_REMINDER}"
        )

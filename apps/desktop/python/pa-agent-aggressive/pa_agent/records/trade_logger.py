...
_CSV_FIELDNAMES = [
    ...
    # ── Image path ────────────────────────────────────────────────────────────
    "chart_image",
    # ── Win-rate ledger (settlement) ──────────────────────────────────────────
    "entry_ts_ms",        # K1 (latest closed bar) open ts, Unix ms UTC — anchors settlement
    "settle_status",      # '' | pending | win | loss | expire | insufficient
    "outcome_detail",     # e.g. 'TP1 hit' / 'SL hit' / 'expire:close_below_entry' / 'no data after entry'
    "settle_bars",        # bars held until settlement trigger
    "settle_price",       # price at settlement trigger
    "settled_at",         # local ISO time of settlement
]


# ── Internal helpers ──────────────────────────────────────────────────────────

def _entry_ts_ms(frame: Any) -> str:
    """K1 (latest closed bar) open timestamp in Unix ms UTC; '' if unknown."""
    try:
        bars = list(getattr(frame, "bars", []) or [])
    except Exception:
        return ""
    for b in bars:
        seq = getattr(b, "seq", None)
        if seq in (1, "1", 1.0):
            ts = getattr(b, "ts_open", None)
            return "" if ts is None else str(int(float(ts)))
    if bars:
        ts = getattr(bars[0], "ts_open", None)
        return "" if ts is None else str(int(float(ts)))
    return ""
...
def _save_trade_record_impl(
    *,
    ...
    row = {
        ...
        "entry_ts_ms": _entry_ts_ms(frame),
        "settle_status": "",
        "outcome_detail": "",
        "settle_bars": "",
        "settle_price": "",

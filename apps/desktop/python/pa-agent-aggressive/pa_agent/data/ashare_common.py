...
#: Tencent real-time quote endpoint. The reply is one ``v_<code>="..."`` line per
#: code, with ``~``-separated fields; index 1 is the display name (GBK encoded).
_TENCENT_QUOTE_URL = "https://qt.gtimg.cn/q={codes}"
_NAME_TIMEOUT_S = 6.0

# Cache resolved names for the process lifetime — a name never changes, and the
# GUI resolves on every keystroke while the user types a code.
_name_cache: dict[str, str] = {}


def lookup_ashare_name(symbol: str) -> str | None:
    """Return the Chinese display name for an A-share code, or None.

    Accepts ``600519`` / ``sh600519`` / ``sh.600519``.  Best-effort: any network
    or parse failure returns None rather than raising, because the only caller is
    a cosmetic label in the toolbar.
    """
    code = normalize_ashare_symbol(symbol)
    if not code:
        return None

    # Tencent wants the market prefix; digits alone are ambiguous (600/601/603/
    # 605/688 are Shanghai, 000/001/002/003/300/301 are Shenzhen).
    if code.startswith(("sh", "sz")):
        query = code
    else:
        query = ("sh" if code[0] in "56" else "sz") + code

    if query in _name_cache:
        return _name_cache[query] or None
    ...
    try:
        ...
        resp = requests.get(
            _TENCENT_QUOTE_URL.format(codes=query),
            headers={"User-Agent": "Mozilla/5.0", "Referer": "https://gu.qq.com/"},
            timeout=_NAME_TIMEOUT_S,
        )
        ...
        logging.getLogger(__name__).debug("A股名称查询失败 %s: %s", query, exc)

    _name_cache[query] = name or ""
    return name

...
class KlineFrame:
    ...
    #: Human-readable instrument name (e.g. 中芯国际 / "Gold vs US Dollar").
    #: Empty when the data source cannot resolve one.  Purely informational.
    symbol_name: str = ""
...
class DataSource(ABC):
    ...
    def resolve_name(self, symbol: str) -> str | None:
        """Return a human-readable display name for *symbol*, or None.

        Optional capability — the toolbar shows the name when a source provides
        one and silently shows nothing otherwise.  Best-effort; must not raise.
        May perform network I/O, so callers should not invoke this on the UI thread.
        """
        return None

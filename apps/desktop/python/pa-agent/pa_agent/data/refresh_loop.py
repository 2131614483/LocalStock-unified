"""1 Hz data refresh loop running on a dedicated thread."""
...
from pa_agent.util.event_bus import Signal
...
class RefreshLoop(threading.Thread):
    ...
    原实现继承 ``QThread``；服务端改为普通守护线程，``run()`` 语义不变。
    ...
    def __init__(
        ...
        # parent 参数仅为兼容原 Qt 构造签名保留，服务端不再需要。
        super().__init__(daemon=True, name="pa-refresh-loop")
        self.frame_ready: Signal = Signal()

...
DataSourceKind = Literal[
    "mt5", "tradingview", "akshare", "eastmoney", "sinatencent", "baostock"
]
...
class GeneralSettings(BaseModel):
    ...
    #: 分析过程模式：original=保留完整二阶段续写上下文；optimized=精简重复上下文以提速
    #: （原项目以 getattr + hasattr 读取该键，字段并不存在，导致此项从未生效；此处补为正式字段）
    analysis_mode: Literal["original", "optimized"] = "original"

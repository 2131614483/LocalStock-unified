...
DB_PATH = os.environ.get('LOCALSTOCK_MARKET_DB') or os.path.join(os.path.dirname(__file__), '..', 'data', 'stock_data.db')
...
class Position:
    ...
    def __init__(self, stock, avg_cost, amount, price):
        ...
        self.closeable_amount = amount  # 可卖量（聚宽字段，A股 T+1 但日频回测按当日可用近似）
...
class Portfolio:
    ...
    @property
    def cash(self):
        """聚宽别名：context.portfolio.cash 实时镜像可用现金"""
        return self._cash
    ...
    def update_prices(self, prices):
        ...
        self.total_value = self._cash + self.positions_value
...
class BacktestEngine:
    ...
    def __init__(self, start_date, end_date, capital=1000000):
        self.start_date = start_date
        self.end_date = end_date
        self.capital = capital
        self.conn = sqlite3.connect(DB_PATH)
        self.conn.row_factory = sqlite3.Row
        self._factor_calc = FactorCalcStdlib(self.conn)
        ...
        self._has_daily_task = False  # 策略是否注册了 run_daily（用于 handle_data 兼容判断）
        self.g = G()
        self.trades = []
        self.daily_records = []
    ...
    def load_stock_data(self, stock_code, start_date=None, end_date=None):
        ...
        if self._is_fund(code):
            ...
            for r in rows:
                ...
                # bfq 日线无除权因子：用 close/pre_close-1 近似 dretwd（基金分红跳空幅度小 <2%/年）
                ret = (close / prev_close - 1) if prev_close and prev_close > 0 else 0.0
                data[str(r['trade_date'])] = {
                    'open': float(r['open']) if r['open'] else close,
                    'close': close,
                    'high': float(r['high']) if r['high'] else close,
                    'low': float(r['low']) if r['low'] else close,
                    'pre_close': prev_close or close,
                    'volume': int(r['volume'] or 0),
                    'amount': float(r['amount'] or 0),
                    'dretwd': ret,
                    'dretnd': ret,   # 基金无单独除息字段，两值相等 → 不分红入账
                    'adj_close_wd': close,
                    'market_type': None,
                    'limit_up': self._derive_limit(prev_close, None, +1),
                    'limit_down': self._derive_limit(prev_close, None, -1),
                }
                prev_close = close
        else:
            rows = self.conn.execute(
                'SELECT trade_date, open_price, close_price, high_price, low_price, '
                'pre_close_price, volume, amount, dretwd, dretnd, adj_close_wd, limit_up, limit_down, market_type '
            ...
            for r in rows:
                ...
                data[str(r['trade_date'])] = {
                    'open': r['open_price'],
                    'close': close,
                    'high': r['high_price'] if (r['high_price'] is not None and r['high_price'] > 0) else close,
                    'low': r['low_price'] if (r['low_price'] is not None and r['low_price'] > 0) else close,
                    ...
                    'limit_up': self._derive_limit(r['pre_close_price'], r['market_type'], +1),
                    'limit_down': self._derive_limit(r['pre_close_price'], r['market_type'], -1),
                }
    ...
    @staticmethod
    def _is_fund(code):
        """基金代码识别：沪 51/56/58/50/52 开头、深 15/16 开头；非 6 位纯股票代码。
        与股票冲突检查：沪市股票 60/68/69、深市 000/001/002/003/300/301/302。"""
        return (code.startswith(('51', '52', '56', '58', '50', '15', '16'))
                and code.isdigit() and len(code) == 6)

    @staticmethod
    def _derive_limit(pre_close, market_type, direction):
        """按板块推导涨跌停价。market_type: 1=沪主板 4=深主板 16=创业板 32=科创板。
        主板 ±10%，创业/科创 ±20%（2020-08-24 后创业板注册制；此前为 10%，
        引擎按当前规则近似——对 2015-2019 创业板的回测该近似略保守，报告中声明）。
        基金（market_type=None）按 ±10%。"""
        if not pre_close or pre_close <= 0:
            return None
        if market_type in (16, 32):
            ratio = 0.20
        else:
            ratio = 0.10
        price = pre_close * (1 + ratio * direction)
        return round(price, 2)

    def _dividend_on_date(self, code, date_str):
        """反推除息日每股现金分红。
        dretwd = (close + div) / prev_close_raw - 1（含分红收益）
        dretnd = close / prev_close_raw - 1（不含分红收益）
        两式相减：dretwd - dretnd = div / prev_close_raw
        ⇒ div_per_share = prev_close_raw * (dretwd - dretnd)
        （茅台 2023-06-30 实测：1713.71 × (0.001867+0.013252) = 25.91 ✓）"""
        data = self._price_cache.get(code, {})
        row = data.get(date_str)
        if not row:
            return 0.0
        dw, dn = row.get('dretwd'), row.get('dretnd')
        if dw is None or dn is None:
            return 0.0
        diff = dw - dn
        if diff <= 1e-6:
            return 0.0
        ...
        if i == 0:
            return 0.0
        prev_close_raw = data[dates[i - 1]]['close']
        if not prev_close_raw or prev_close_raw <= 0:
            return 0.0
        div = prev_close_raw * diff
        # 防异常：单日分红超过股价 30% 视为数据噪声（送转股等非现金事件），忽略
        if div > prev_close_raw * 0.3:
            return 0.0
        return max(div, 0.0)
    ...
    def api_attribute_history(self, stock, count, unit='1d', fields=('close',)):
        ...
        for field in fields:
            field_map = {
                ...
                'high': 'high', 'low': 'low',  # P1 回填后用真实高低价（缺失自动回退 close）
                'money': 'amount',
            }
    ...
    def api_order_target_value(self, stock, value):
        ...
        # A股真实约束：涨停买不进、跌停卖不出（按当日收盘价是否触及推导的涨跌停价判断）
        data = self._price_cache.get(code, {})
        bar = data.get(current_date)
        if bar:
            lu, ld = bar.get('limit_up'), bar.get('limit_down')
            if lu is not None and price >= lu - 1e-9 and value > 0:
                self.log.warn(f'{stock} 涨停无法买入，跳过')
                return None
            if ld is not None and price <= ld + 1e-9 and value < 0:
                self.log.warn(f'{stock} 跌停无法卖出，跳过')
                return None
    ...
    def api_run_daily(self, fn, time='09:30'):
        if time in ('every_bar',):
            time = '09:30'
        self.scheduled_tasks.append(('daily', fn, time))
        self._has_daily_task = True
    ...
    def _fmt_stock(self, code):
        return code + (".XSHG" if code.startswith(("6", "9")) else ".XSHE")

    def api_get_all_stocks(self, date_str=None):
        d = date_str or self.context.current_dt.strftime("%Y-%m-%d")
        return [self._fmt_stock(c) for c in self._factor_calc.get_all_stocks(d)]

    def api_get_factors(self, stocks, factor_names, date_str=None):
        d = date_str or self.context.current_dt.strftime("%Y-%m-%d")
        codes = [s.split(".")[0] for s in stocks]
        res = self._factor_calc.get_factors(codes, list(factor_names), d)
        return {s: res.get(c, {}) for s, c in zip(stocks, codes)}

    def api_get_factor(self, stock, factor, date_str=None):
        return self.api_get_factors([stock], [factor], date_str).get(stock, {}).get(factor)

    def api_rank_normalize(self, values, ascending=True):
        r = self._factor_calc.rank_normalize(values)
        return {k: -v for k, v in r.items()} if not ascending else r

    def api_order_target_percent(self, stock, pct):
        return self.api_order_target_value(stock, self.context.portfolio.total_value * pct)
    ...
    def run(self, code_str):
        ...
        env = {
            ...
            'get_all_stocks': self.api_get_all_stocks,
            'get_factor': self.api_get_factor,
            'get_factors': self.api_get_factors,
            'rank_normalize': self.api_rank_normalize,
            'order_target_percent': self.api_order_target_percent,
            'record': lambda *a, **kw: None,   # 聚宽可视化指标记录，本地空操作
            'log': self.log,
            'g': self.g,
            'FixedSlippage': lambda v: Slippage(v),
            'OrderCost': lambda close_tax=0.001, open_commission=0.0003,
                          close_commission=0.0003, min_commission=5,
                          close_today_commission=None, open_tax=None: OrderCost(
        ...
        try:
            import importlib
            jq = importlib.import_module('jqcompat')
            exec_env = jq.merge_jqcompat(exec_env, self)
        except Exception as e:
            self.log.warn(f'jqcompat 加载失败（不影响基础回测）: {e}')
        try:
        ...
        for date_str in trade_days:
            ...
            # 同步持仓可卖量（T+1 简化：当日买入不可卖 —— 追加后 amount==closeable_amount 已有，此处统一）
            for pos in self.context.portfolio.positions.values():
                pos.closeable_amount = pos.amount

            # 除息日分红/送转入账：按 dretwd 与 dretnd 之差反推每股现金红利，
            # 以当日收盘价折算成现金加回（等价于红利再投资的近似，保守不放大收益）
            for stock in list(self.context.portfolio.positions.keys()):
                code = stock.split('.')[0]
                div_ps = self._dividend_on_date(code, date_str)
                if div_ps > 0:
                    pos = self.context.portfolio.positions.get(stock)
                    if pos and pos.amount > 0:
                        cash = div_ps * pos.amount
                        self.context.portfolio.available_cash += cash
                        self.log.info(f'{stock} 除息分红: 每股 {div_ps:.4f} 元 x {pos.amount} 股 = {cash:.2f} 元入账')
            ...
            # 兼容老版策略：handle_data(context, data) 每日调用（无 run_daily 注册时）
            if 'handle_data' in exec_env and not self._has_daily_task:
                try:
                    exec_env['handle_data'](self.context, {})
                except TypeError:
                    # 某些策略 handle_data 只接收 context
                    try:
                        exec_env['handle_data'](self.context)
                    except Exception as e2:
                        self.log.error(f'handle_data 错误: {e2}')
                except Exception as e:
                    self.log.error(f'handle_data 错误: {e}')
